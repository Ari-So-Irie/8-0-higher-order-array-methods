...
function getSongsBySaib(songs) {
  let songArtist = songs.filter((song)=> song.artist === "Saib")
  return songArtist
}
...
function getSongsOverThreeMinutes(songs) {
  let overThree = songs.filter((song)=> song.runtimeInSeconds > 180)
  return overThree
}
...
function getTitleTracks(songs) { 
  let titleTracks = songs.filter((song)=> song.title === song.album)
  return titleTracks}
